...
class wpForoTemplate{
	...
	function topic_form($forumid = 0, $type = 'default', $layout = NULL ){
		...
		$editor_id = 'wpf-topic-create-' . $forumid;
        ?>
		<div id="<?php echo $editor_id ?>-wrap" class="wpf-topic-create <?php echo esc_attr( 'wpf-topic-editor-' . $type ); ?>">
			<form name="topic" action="" enctype="multipart/form-data" method="POST" class="wpforoeditor">
                ...
                <?php if( $forumid && $type == 'default' ): ?>
                    <input type="hidden" id="wpf_parent" name="topic[forumid]" value="<?php echo intval($forumid) ?>" />
                <?php else: ?>
                    <div class="wpf-topic-forum-field" style=" padding: 0px 10px 15px 10px; text-align: center; margin: 0px auto 10px auto;">
                ...
                <div class="wpf-topic-form-wrap" <?php if($type != 'default') echo 'style="display: none;"' ?>>
                    <label class="wpf-subject-label" style="padding-left:8px;"> <?php wpforo_phrase(( WPF()->forum->get_layout() == 3 ? 'Your question' : 'Topic Title' )) ?> * </label>
                    ...
                    <?php
                        $settings = WPF()->tpl->editor_buttons( 'topic' );
	...
	*/
	function reply_form($args){
		...
		<div id="wpf-form-wrapper" <?php echo ( WPF()->forum->get_layout() == 3 && !$this->forms['qa_display_answer_editor'] ? 'style="display: none;"' : '' ) ?>>
			...
			<div id="wpf-post-create" class="wpf-post-create">
				<form name="post" action="" enctype="multipart/form-data" method="POST" class="wpforoeditor">
					<?php wp_nonce_field( 'wpforo_verify_form', 'wpforo_form' ); ?>
					...
					<?php
					$editor_id = 'wpf-reply-create-' . $forumid;
                    $settings = WPF()->tpl->editor_buttons( 'post' );
					wp_editor( '', $editor_id, $settings );
					?>
					<div class="wpf-extra-fields">
	...
	public function post_form_simple(){
        ...
        ?>
        <form enctype="multipart/form-data" method="POST" class="wpforo-post-form" style="display: none;">
	        <?php wp_nonce_field( 'wpforo_verify_form', 'wpforo_form' ); ?>
	        ...
	        <?php endif; ?>
            <div class="wpf_post_form_textarea_wrap"  data-textaretype="simple_editor">
                <textarea required class="wpf_post_body" name="post[body]" placeholder="<?php wpforo_phrase('Write here . . .') ?>"></textarea>
            </div>
            <div class="wpf-extra-fields">
	...
	public function post_form(){
        ...
        <form enctype="multipart/form-data" method="POST" class="wpforo-post-form" style="display: none;">
	        ...
	        <?php endif; ?>
            <div class="wpf_post_form_textarea_wrap wpf-post-create" data-textaretype="rich_editor">
                <textarea class="wpf_post_body" name="post[body]"></textarea>
            </div>
            <div class="wpf-extra-fields">
		        <?php do_action('wpforo_portable_form_extra_fields_before') ?>
		        <?php do_action('wpforo_portable_form_buttons_hook'); ?>
		...
		foreach($buttons as $button){
			...
			switch($button){
				...
				case 'reply': 
					...
					if( WPF()->perm->forum_can('cr', $forumid) ){
					    ...
					    $layout_class = 'wpforo_layout_' . intval($layout);
			   			$button_html[] = '<span id="parentpostid'.wpforo_bigintval($postid).'" class="wpforo-reply wpf-action '.$layout_class.'"><i class="fas fa-reply fa-rotate-180"></i><span class="wpf-button-text">' . wpforo_phrase('Reply', false).'</span></span>';
			   		}else{
				...
				case 'answer':
					...
					if( WPF()->perm->forum_can('cr', $forumid) ){
			   			if( WPF()->topic->can_answer($topicid) ){
                            $button_html[] = '<span class="wpforo-answer wpf-button" data-phrase="' . esc_attr( wpforo_phrase('Answer', false) ).'"><i class="fas fa-pencil-alt"></i><span class="wpf-button-text">' . wpforo_phrase('Answer', false).'</span></span>';
                        }
			   		} else {
				...
				case 'quote':
					if($is_closed || $is_approve) break;
					if( WPF()->perm->forum_can('cr', $forumid) ) {
						$button_html[] = '<span wpf-tooltip="'.esc_attr(wpforo_phrase('Quote', false)).'" class="wpf-action wpforo-quote" data-postid="'.wpforo_bigintval($postid).'"><i class="fas fa-quote-left wpfsx"></i><span class="wpf-button-text">' . wpforo_phrase('Quote', false).'</span></span>';
			   		}else{
