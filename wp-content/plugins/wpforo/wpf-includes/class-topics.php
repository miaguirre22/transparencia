		...
		if( WPF()->db->insert(
			...
			if( WPF()->db->insert(
				...
				if( FALSE !== WPF()->db->update(
					...
					if( !$status && !$private) WPF()->db->query( "UPDATE ".WPF()->tables->forums." SET `last_post_date` = '" . esc_sql( $created ). "', `last_userid` = " . wpforo_bigintval( $userid ). ", `last_topicid` = " . wpforo_bigintval( $topicid ) . ", `last_postid` = " . wpforo_bigintval( $first_postid ) . ", `topics` = `topics` + 1, `posts` = `posts` + 1 WHERE `forumid` = " . intval($forumid) );
